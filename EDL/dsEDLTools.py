...
class MyForm(base_class, form_class):
    ...
    def epiCutup(self):
        ...
        for x in sorted(self.epiDict):
            seqVal =x.split("_")
            fullPath = self.dsPipe + str(self.cb_Project.currentText()) + "/film/"+ str(self.cb_Episode.currentText()) + "/"
            print fullPath
            
            dsFS.dsCreateFs("3D",fullPath,seqVal[0])
            ...
            for y in sorted(self.epiDict[x]):
                ...
                self.qtThumb(qtIn,qtOut,jpgOut,tcIn,tcOut)
                ...
                fullPath = self.dsComp + str(self.cb_Project.currentText()) + "/film/"+ str(self.cb_Episode.currentText()) + "/"+ tmpVal[0]+"/"
    ...
    def updateSG(self,shotName,qtOut,jpgOut,qtIn):
        ...
        if self.sg.find_one("Sequence", [['code','is', seqName], ['project.Project.name','is', str(projName)],['scene_sg_sequence_1_scenes.Scene.code','is', str(epiName)]],['code']) == None:
            ...
            mySeq = self.sg.create('Sequence', data)
        ...
        if self.sg.find_one("Shot", [['code','is', shotName], ['project.Project.name','is', str(projName)],['sg_sequence.Sequence.code','is', str(seqName)],['sg_scene.Scene.code','is', str(epiName)]],[]) == None:
            ...
            myShot = self.sg.create('Shot', data)
        ...
        if self.sg.find_one("Version", [['code','is', "anim"], ['project.Project.name','is', str(projName)],['entity.Shot.code','is', str(shotName)],['sg_sequence.Sequence.code','is', str(seqName)]],[]) == None:
            filters = [ ['code','is', 'DucklingShotTemplate' ]]
            template = self.sg.find_one('TaskTemplate',filters)

            data = {'code':"animatic",'image':str(jpgOut),'project':myProj,'entity':myShot,'sg_path_to_movie':str(qtOut)}
            sgVersion = self.sg.create('Version', data)
            
            try:self.te_log.appendPlainText("created shot in shotgun")
            except:pass
        else:
            self.sg.update('Version',myShot['id'], {'sg_path_to_movie':str(qtOut),'image':str(jpgOut)})
    ...
    def adjustTC(self,tc,offset):
        ...
        SS = int(tcSplit[2]) - int(offSS[2])
        FF = int(tcSplit[3]) - int(offSS[3])
        return '%02d:%02d:%02d:%02d' % (HH,MM,SS,FF)

    def qtThumb(self,qtIn,qtOut,jpgOut,tcIn,tcOut):
        # Create Icon 
        cmd = self.rvIO + " " + str(qtIn) + " -t " + str(tcIn) + "-" + str(tcIn) + " -o " + jpgOut
...
#                shotDur = int(shoDur/1000)
                item = QtGui.QTableWidgetItem(shotDur)
                self.tw_sqsh.setItem(rowCount,5,item)
                
                #print shotName + "    "+str(self.adjustTC(shotStart,self.tcOffset))+"    "+str(self.adjustTC(shotEnd,self.tcOffset)) + "    " + shotDur
                
                self.te_log.appendPlainText(shotName + "    "+str(self.adjustTC(shotStart,self.tcOffset))+"    "+str(self.adjustTC(shotEnd,self.tcOffset)) + "    " + shotDur)
    ...
    def loadQT(self):
        #load qt file
        self.qtFile = QtGui.QFileDialog.getOpenFileName(self, "Open Data File", "", "Quicktime MOV (*.mov)")
