...
class publishStandalone(form_class, base_class):
    ...
    def __init__(self, parent=None):
        ...
        if sys.platform == "linux2":
            ...
            self.rvio = "'/usr/local/rv-Linux-x86-64-3.12.20/bin/rvio'"
            self.rv = "'/usr/local/rv-Linux-x86-64-3.12.20/bin/rv'"
            self.rvpush = "'/usr/local/rv-Linux-x86-64-3.12.20/bin/rvpush'"
            ...
            self.rv = '"C:/Program Files (x86)/Tweak/RV-3.12.20-32/bin/rv.exe"'
            self.rvpush = '"C:/Program Files (x86)/Tweak/RV-3.12.20-32/bin/rvpush.exe"'
    ...
    def versionUp(self):
        ...
        lns = self.nukeScripts_LW.currentItem().text()
        print lns
            ...
            if os.path.exists(src):
                ...
                if sys.platform == "linux2":
                    ...
                    if self.actionQT.isChecked():
                        ...
                        path = dstPY+ "/QT.py"
                        bFile = open(path, 'w')
                        bFile.write("import shutil,subprocess\nproc = subprocess.call( '"+ cmd +"')\nshutil.copy('"+ dstQT + "/" + frameMov + "','" + localQT + "/" + frameMov + "')")
                        bFile.close()
                        
            pyList = os.listdir(dstPY)
            for py in pyList:
                print "Running " + dstPY + "/" + py
                cmd = dstPY + "/" + py
                if sys.platform == "linux2":
                    self.process(cmd)
                else:
                    proc = subprocess.Popen(['python',cmd],shell=False,stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,)
    
            renderFile = rndFile
            #pubPath = '%s/%s/film/%s/%s/%s/comp/compOut/' % ( self.dsComp,pr, ep, sq, sh)
            pubPath = 'dsComp/%s/film/%s/%s/%s/comp/compOut/' % (pr, ep, sq, sh)
    ...
    def copyFrames(self,src,dstDPX):   
        ...
        for tmp in tmpList:
            if not tmp.endswith(".nk"):
                if not tmp.endswith(".mov"):
                    if not tmp.endswith(".jpg"):
                        print "working on " + dstRoot + tmp
                        
                        #shutil.copy2(srcRoot,dstRoot )
                        shutil.copy(srcRoot + "/" + tmp, dstRoot + tmp)
                        
                        #pid = shutil.copy(srcRoot + "/" + tmp, dstRoot + tmp)
                        #pid_list.append(pid)
                        #print filter(lambda x:x.poll())
                        #while len(filter(lambda x: x.poll() is None, pid_list)) >= 12:
                            #time.sleep(.5)
    ...
    def open_nukeScript(self):
        ...
        if sys.platform == "linux2":
            ...
            cmd = '/usr/local/Nuke7.0v6/Nuke7.0' + ' ' +  str(pathNS)
            self.process(cmd)
            #subprocess.Popen(cmd, creationflags = subprocess.CREATE_NEW_CONSOLE)
            ...
            cmd = 'C:/Program Files/Nuke7.0v6/Nuke7.0.exe' + ' ' +  str(pathNS)
            subprocess.Popen(cmd, creationflags = subprocess.CREATE_NEW_CONSOLE)
