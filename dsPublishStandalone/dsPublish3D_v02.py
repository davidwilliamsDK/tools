...
class publishStandalone(form_class, base_class):
    ...
    def publish(self):
        ...
        for shot in self.shots.selectedItems():
            sh = shot.text()
            dstRoot = r'%s/%s/film/%s/%s/%s/' % (self.dsPipe,pr, ep, sq, sh)
                ...
                try:
                    ...
                    if os.path.exists(src):
                        ...
                        if len(frameList) > 0:
                            ...
                            self.copyFrames(name, src, dst, start, end, extension,dstPY)
                    
                            print "Talking with Shotgun"
                            cmd =  'sgTools.sgPublish3DFrames(\''+ str(fulsrc)+ '\',\'' + str(rndFile) + '\',\'' + str(us) + '\',\''+ str(pubPath)+'\')'
                            dsSP.spSG('sgTools.sgPublish3DFrames(\''+str(fulsrc)+'\',\''+str(rndFile)+'\',\''+ str(us)+'\',\''+ str(pubPath)+'\')' )
    ...
    def copyFrames(self, name, src, dst, start, end, extension,dstPY):
        '''
        Copy frames from src to dst.
        '''
        srcSplit = src.split("/")
        renderLayer = srcSplit[-1]
        
        path =  dstPY + "/" + str(renderLayer) +"_COPY.py"
        #//vfx-data-server/dsPipe/Lego_ChimaFillers/film/01_AirHeadFlightTest_011179/q0010/s0190/rawRender/smoke_RL/smoke_RL_2036.exr
        "\\vfx-data-server\dsPipe\Lego_ChimaFillers\film\01_AirHeadFlightTest_011179\q0010\s0190\rawRender\smoke_RL\smoke_RL_2036.exr"
        print path
        bFile = open(path, 'w')
        ...
        for number in range(int(start), int(end)+1):
            ...
            if os.path.exists(input):
                ...
                bFile.write('shutil.copy2( \"'+str(input)+'\", \"' + str(output) + '\")\n')
                bFile.write('print \" copying '+str(input)+' ::: ' + str(output) + '\"\n')
    ...
    def rrSubmit(self, nukeScript, startFrame, endFrame):
        ...
        if sys.platform == "linux2":
            ...
            cmd = "//vfx-render-manager/royalrender/bin/win/rrSubmitterconsole.exe %s -SeqStart %s -SeqEnd %s %s" % (nukeScript, startFrame, endFrame, globals)
